...
/// A discovered `aide.yml`, alongside the directory it lives in (jobs keep
/// their `aide.yml`, prompt file, and eventually `runtime.yml` side by
...
/// Recursively glob `workspace` for `aide.yml` files and parse each one.
...
pub fn scan_workspace(workspace: &Path) -> anyhow::Result<ScanResult> {
    let pattern = format!("{}/**/aide.yml", workspace.display());
    let mut jobs = Vec::new();
    let mut failed = Vec::new();

    for entry in glob::glob(&pattern)? {
        let spec_path = entry?;
        match AideJob::load(&spec_path) {
            Ok(job) => {
                let dir = spec_path
                    .parent()
                    .map(|p| p.to_path_buf())
